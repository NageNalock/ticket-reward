...
窗口显示最新正式版本、更新说明和安装包大小。点击「下载更新」后可查看进度或取消下载。应用优先使用 GitHub 提供的 `sha256` digest；没有 digest 时读取同一 Release 的 `SHA256SUMS.txt`。只有大小和 SHA-256 都匹配的完整 ZIP 才会出现在最终路径中。

下载完成后点击「在 Finder 中显示」，解压 ZIP，退出应用，再把新版 `Bing Rewards.app` 拖入「应用程序」替换。应用不会直接覆盖正在运行的程序。
...
- 自动选择 `arm64`、`x86_64` 或 `universal2` / `universal` ZIP。缺少匹配架构的包时显示提示。
...
dist/Bing-Rewards-macOS-arm64.zip
```
